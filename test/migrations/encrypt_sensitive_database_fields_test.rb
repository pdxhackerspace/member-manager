...
class EncryptSensitiveDatabaseFieldsTest < ActiveSupport::TestCase
  ...
  test 'backfill does not disturb members it only re-encodes' do
    user = users(:one)
    connection = ActiveRecord::Base.connection
    connection.execute(<<~SQL.squish)
      UPDATE users
      SET email = 'settled-user@example.com',
          mailing_address = '99 Settled Way',
          mailing_latitude = 45.5,
          mailing_longitude = -122.6,
          mailing_geocoded_at = NOW(),
    ...
    assert_equal 'settled-user@example.com', user.email
    assert_in_delta 45.5, user.mailing_latitude
    assert_in_delta(-122.6, user.mailing_longitude)
