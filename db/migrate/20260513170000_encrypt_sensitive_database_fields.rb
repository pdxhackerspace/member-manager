class EncryptSensitiveDatabaseFields < ActiveRecord::Migration[8.1]
  ...
  # Re-encoding a value at rest is not a business change, so this writes columns directly.
  # save! would fire the model callbacks, and because each encryption uses a fresh nonce
  # every row looks changed: member coordinates would be cleared and re-geocoded, records
  # flagged for Authentik sync, and a journal entry written for every member.
  def encrypt_records(model_class, *attributes)
    ...
    model_class.find_each do |record|
      ...
      apply_lookup_digests(record)

      updates = record.changes.transform_values(&:last)
      record.update_columns(updates) if updates.any?
    end
  end

  # The digest columns are normally filled by model callbacks, which update_columns skips.
  # Each is named after the field it indexes, so <field>_lookup_digest finds its own source.
  def apply_lookup_digests(record)
    record.attribute_names.each do |name|
      next unless name.end_with?('_lookup_digest')

      source = name.delete_suffix('_lookup_digest')
      next unless record.respond_to?(source)

      record[name] = SensitiveData.email_digest(record.public_send(source))
    end

    return unless record.has_attribute?('extra_email_lookup_digests')

    record[:extra_email_lookup_digests] = SensitiveData.email_digests(record.extra_emails)
  end
end
